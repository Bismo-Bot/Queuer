...
function playHandler(message) {
	...
	const voiceChannel = message.message.member.voice.channel;
	if (!voiceChannel)
	...
	function YouTubeGetID(url){
	   ...
	   return (url[2] !== undefined) ? url[2].split(/[^0-9A-Za-z_\-]/i)[0] : url[0];
	}

	function cont(link) {
		Bismo.log("Playing video ID: " + link);
...
function main(Requests) {
	...
	Bismo.registerCommand("yt", playHandler, "Play YouTube videos in voice chat!", "Usage:\n"
